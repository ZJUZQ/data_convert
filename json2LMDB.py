...
def writeLMDB(datasets, lmdb_path, validation):
	...
	txn = env.begin(write=True) # create a transaction
	data = []
	numSample = 0
	...
	for d in range(len(datasets)):
		if(datasets[d] == "MPI"):
			...
			#print data
			print numSample	
		elif(datasets[d] == "Undef"): # for self dataset
			print datasets[d]
			with open( os.path.expanduser("~") + '/data_convert/Undef/json.json' ) as data_file:
				data_this = json.load(data_file)
				data = data + data_this # note, we do not have 'root' field
			numSample = len(data)
			#print data
	...
	print 'totalWriteCount', totalWriteCount
	writeCount = 0
	...
	for count in range(numSample):
		...
		elif "Undef" in data[idx]['dataset']:
			path_header = os.path.expanduser("~") + '/data_convert/Undef/images/'

		#print os.path.join(path_header, data[idx]['img_paths'])
		#print os.path.join(path_header, data[idx]['img_paths'].split('/')[-1])
		img = cv2.imread(os.path.join(path_header, data[idx]['img_paths'].split('/')[-1]))
		#print data[idx]['img_paths']
		if "Undef" in data[idx]['dataset']:
			img_idx = data[idx]['img_paths'].split('/')[-1].split('.')[0]
		else:
		...
		if(nop!=0):
			if(nop==1):
				...
				objpos_other = data[idx]['objpos_other']
				scale_provided_other = data[idx]['scale_provided_other']
			#print 'objpos_other', objpos_other
...
if __name__ == "__main__":
	...
	writeLMDB(['Undef'], os.path.expanduser("~") + '/data_convert/lmdb', 0)
